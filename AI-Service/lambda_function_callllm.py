...
class Config:
    ...
    MAX_FILE_SIZE_BYTES: int = int(4.5 * 1024 * 1024)  # ~4.5MB
...
def parse_contract_input(data: Dict[str, Any]) -> ContractInput:
    ...
    return ContractInput(language=language, contract_text=contract_text)


# -----------------------------------------------------------------------------
# 5. RAG hook (stub) - sẽ mở rộng sau
# -----------------------------------------------------------------------------

def retrieve_legal_context(contract_text: str, language: str) -> str:
    """
    Gọi RAG-search API /rag/search, trả về text context để nhét vào prompt LLM.
    ...
    payload = {
        ...
        "filters": {
            "source_type": ["legal"],
            # Nếu anh muốn chỉ lấy lĩnh vực 'Xây dựng - Đô thị'
    ...
    try:
        ...
        print(f"[WARN] RAG request failed: {e}")
        return ""
...
def call_bedrock_document(
    ...
    # Lưu ý: context từ RAG với document sẽ xử lý sau; tạm thời bỏ qua để tránh prompt quá nặng.
    logger.info(
        "Calling Bedrock model %s with DOCUMENT (format=%s, name=%s, size=%d bytes) ...",
        Config.MODEL_ID, file_format, file_name, len(file_bytes)
    )
...
def parse_model_json(model_output_text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    """
    Parse JSON từ LLM. Nếu lỗi, trả analysis fallback + raw_model_output.
    """
    try:
        analysis = json.loads(model_output_text)
        return analysis, None
    except json.JSONDecodeError:
        logger.warning("Model output is not valid JSON, returning raw text")
        analysis = {
            "summary": "",
            "overall_risk_level": "UNKNOWN",
            "risk_items": [],
            "disclaimer": "Kết quả không ở dạng JSON chuẩn. Vui lòng kiểm tra lại."
        }
...
def analyze_contract(contract_input: ContractInput) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    if contract_input.has_file:
        ...
        # Trường hợp này thực tế không xảy ra do đã check ở parse_contract_input
...
def make_response(status_code: int, body: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "headers": {
            ...
            "Access-Control-Allow-Origin": "*",  # demo, sau này giới hạn domain Amplify
        },
